...
function sendScanRequest(text) {
    updateUI('Sending text to Custom AI Agent...', true);

    chrome.runtime.sendMessage({ action: "scanText", text: text }, (response) => {
        if (chrome.runtime.lastError) {
            console.error("Runtime error:", chrome.runtime.lastError.message);
            updateUI('Error communicating with service worker. Is the Python API running?', false);
            return;
        }
        ...
        if (response.error) {
            ...
            // Notifications are handled by background.js, but we display the score here
            displayResults(response.summary.ai, response.summary.human, response.suggestions);
        } else {
...
// Function to display the colorful results
function displayResults(aiScore, humanScore, suggestions) {
